...
#define EXECUTE(get_slicer,slicer_data,lower2,upper2,k_offset,step2,chunk2)\
   for ( i = 0; i < NUM_ITERS; i++ ) {\
      ...
      _loop_data.loop_info.lower = upper2 + k_offset; \
      ...
      sys.submit( *wd );\
      wg->waitCompletion();\
      if ( step2 > 0 ) for ( int j = upper2+k_offset; j <= lower2+k_offset; j+= step2 ) { fprintf(stderr,"Test Fail!\n"); A[j+_loop_data.offset]--; }\
      else if ( step2 < 0 ) for ( int j = upper2+k_offset; j >= lower2+k_offset; j+= step2 ) { fprintf(stderr,"Test Fail!\n"); A[j+_loop_data.offset]--; } \
   }
