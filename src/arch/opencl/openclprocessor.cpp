...
OpenCLProcessor::OpenCLProcessor( int id, int devId ) :
   CachedAccelerator( id, &OpenCLDev ),
   _openclAdapter(),
...
WD & OpenCLProcessor::getMultiWorkerWD () const { 
   fatal( "getMultiWorkerWD: OpenCLProcessor is not allowed to create MultiThreads" );
}

BaseThread & OpenCLProcessor::createMultiThread ( WorkDescriptor &wd, unsigned int numPEs, PE **repPEs ) {
   fatal( "OpenCLProcessor is not allowed to create MultiThreads" );
}

BaseThread & OpenCLProcessor::createThread ( WorkDescriptor &wd, SMPMultiThread *parent ) {
   fatal( "getMultiWorkerWD: OpenCLProcessor does not support MultiThreads" );
}
