...
void SlicerStaticFor::submit ( SlicedWD &work )
{
   ...
   if ( _chunk == 0 ) {
      ...
      _chunk = ((_niters / num_threads) -1 ) * _step;
...
// FIXME SMP-only
   ...
   BaseThread *mythread = myThread;
   ThreadTeam *team = mythread->getTeam();
   int first_valid_thread = 0, i, j, valid_threads = 0, num_threads = team->size();
   WorkDescriptor *slice = NULL;
   nanos_loop_info_t *nli;
    ...
    *     threads (0 & 3)
    *
    *     - valid_threads = 4, first_valid_thread = 1 and:
    *
    *                       0    1    2    3    4    5
    *                    +----+----+----+----+----+----+
   ...
   if ( _chunk == 0 ) {
      ...
      for ( i = 1; i < valid_threads; i++ ) {
         ...
         slice->tieTo( (*team)[j] );
         // Scheduler::submit ( *slice );
         (*team)[j].setNextWD(slice); // FIXME: prova temporal?
      }
   } else {
      // Computing offset between threads
      int _sign = ( _step < 0 ) ? -1 : +1;
   ...
   //(*team)[first_valid_thread].setNextWD(&work); // FIXME: prova temporal ?
#endif
