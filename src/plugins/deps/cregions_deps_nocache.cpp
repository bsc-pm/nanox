   ...
   namespace ext {
      ...
      class CRegionsDependenciesDomain : public BaseDependenciesDomain
      {
         ...
         private:
             ...
             */
            std::vector<TrackableObject* >& lookupDependency ( const DepsRegion& target  )
            {
...
//               }               unsigned int vectorIdx = 0;
               ...
               //And address Maps
               if ( itCache == _addressDependencyCache.end() ) { 
                   std::vector<TrackableObject*>* new_objs=NEW std::vector<TrackableObject*>();
                   TrackableObject* status = NEW TrackableObject();
                   _addressDependencyVector.push_back( std::make_pair( target, status ) );
                   new_objs->push_back(status);
...
//                        if ( _addressDependencySmall.find(target) == _addressDependencySmall.end() ){
//                           TrackableObject* status = NEW TrackableObject(); 
//                           _addressDependencySmall.insert( std::make_pair( target, status ) );
//                        }
//                    }
//                    
...
//                        if ( _addressDependencyMid.find(target) == _addressDependencyMid.end() ){
//                           TrackableObject* status = NEW TrackableObject(); 
//                           _addressDependencyMid.insert( std::make_pair( target, status ) );
//                        }
//                    }
//                    //All the addresses starting lower than startAddr-threshold do not collide
...
//               }
         ...
         protected:
            ...
            void submitDependableObjectDataAccess( DependableObject &depObj, DepsRegion &target, AccessType const &accessType, SchedulePolicySuccessorFunctor* callback, TR1::unordered_map<TrackableObject*, bool>& statusMap )
            {
               ...
               std::vector<TrackableObject*>& objs =lookupDependency( target );
               ...
               for ( ; it != objs.end(); ++it) {
                    ...
                    if ( iterStat == statusMap.end() ){  
                        ...
                        //This region was marked as "input" in this task, but our current writer 
                        //has to wait for it until all readers finish, so remove myself from the dependency
                        //add wait for all readers and then set me as reader again
         ...
         public: 
            CRegionsDependenciesDomain() : BaseDependenciesDomain(), _addressDependencyVector(), _addressDependencyCache(), _sizeThresholdSmall( 500 ),
            _sizeThresholdMid( 50000 ){
