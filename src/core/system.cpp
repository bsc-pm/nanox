...
void System::DEEP_Booster_alloc(MPI_Comm comm, int number_of_spawns, MPI_Comm *intercomm) {  
    ...
    if ( !nanos::ext::MPIProcessor::_mpiHosts.empty() ){   
        std::stringstream hostInput(nanos::ext::MPIProcessor::_mpiHosts);
        std::string line;
        while( getline( hostInput, line , ';') ){
            tmp_storage.push_back(line);
        }
    } else if ( !nanos::ext::MPIProcessor::_mpiHostsFile.empty() ){
        std::ifstream infile(nanos::ext::MPIProcessor::_mpiHostsFile.c_str());
    ...
    if ( !nanos::ext::MPIProcessor::_mpiExecFile.empty() ){   
        result_str=nanos::ext::MPIProcessor::_mpiExecFile;
    } else {
        ...
        if (!nanos::ext::MPIProcessor::_mpiFilename.empty()){
            result_tmp=nanos::ext::MPIProcessor::_mpiFilename;
    ...
    for ( int i=0;i<number_of_spawns;i++ ){
        ...
        array_of_commands[i]=const_cast<char*> (nanos::ext::MPIProcessor::_mpiLauncherFile.c_str());
        n_process[i]=1;
