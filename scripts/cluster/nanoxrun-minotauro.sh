...
function runApp
{
   if [ x$USE_MPI = xyes ] ; then
      ...
      if [ x$INTERACTIVE = xyes ] ; then
         ...
         CMD="srun --resv-ports $WRAPPER_CMD $VALGRIND_CMD ./$APP $APP_ARGS"
      fi
   else
...
function makeCmd
{
   ...
   fi
   {
      ...
      echo \#@ initialdir = .
      echo \#@ output = otpt.$FILEID.$NX_ARGS_FOR_FILE_NAME.%j.out
      echo \#@ error= otpt.$FILEID.$NX_ARGS_FOR_FILE_NAME.%j.err
      echo \#@ nodes = $NUM_NODES
      echo \#@ total_tasks = $NUM_NODES
      echo \#@ cpus_per_task = 12
      #echo \#@ nodeset= clos
      echo \#@ wall_clock_limit = $JOB_TIME
      #echo \#@ tracing = 1
      #if [ x$DISABLE_CUDA = x ] ; then
         echo \#@ gpus_per_node = 2
      #fi
      echo nanoxrun.sh --no-interactive $WRAPPER_FLAG $EXTRA_NX_ARGS $DISABLE_CUDA -- $APP $APP_ARGS
      echo rm $CMDFILE
   } > $CMDFILE
#cat $CMDFILE
}
