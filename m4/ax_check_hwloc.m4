...
AS_IF([test "x$with_hwloc" != xno], [
   ...
   # Get include paths
   AS_IF([test -d "$with_hwloc_include"], [
      hwlocinc="-I$with_hwloc_include"
      hwloc_h="$with_hwloc_include"
   ], [test -d "$with_hwloc" && test -d "$with_hwloc/include"], [
      hwlocinc="-I$with_hwloc/include"
      hwloc_h="$with_hwloc/include/hwloc.h"
   ])

   # Get library paths
   AS_IF([test -d "$with_hwloc_lib"], [
      hwloclib="-L$with_hwloc_lib -Wl,-rpath,$with_hwloc_lib"
   ], [test -d "$with_hwloc" && test -d "$with_hwloc/lib64"], [
      hwloclib="-L$with_hwloc/lib64 -Wl,-rpath,$with_hwloc/lib64"
   ], [test -d "$with_hwloc" && test -d "$with_hwloc/lib"], [
      hwloclib="-L$with_hwloc/lib -Wl,-rpath,$with_hwloc/lib"
   ])


   # Check if provided headers and libraries are usable and correct
   AC_LANG_PUSH([C++])
   ...
   AS_IF([test "x$hwloc" != xyes], [
      AS_IF([test "x$with_hwloc" = xcheck], [
         dnl no error
      ], [test "x$with_hwloc" = xyes], [
         AC_MSG_ERROR([
------------------------------
hwloc cannot be detected
Please, provide a suitable installation.
------------------------------])
      ], [
         AC_MSG_ERROR([
------------------------------
...
------------------------------])
      ...
      AS_IF([test "x$ac_cv_hwloc_version" = x], [
         AC_MSG_ERROR([
------------------------------
Could not find hwloc package version
